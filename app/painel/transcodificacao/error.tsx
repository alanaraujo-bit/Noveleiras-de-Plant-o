...
export default function ErroDaTranscodificacao({
  error,
  retry,
}: {
  ...
  useEffect(() => {
    console.error("Falha ao carregar a fila", error);
  }, [error]);

  return (
    <>
      <Cabecalho
        titulo="Transcodificação indisponível"
        descricao="Não foi possível consultar a fila de preparação de vídeo agora."
      />
      <Conteudo>
        <section className="painel-cartao px-5 py-10 text-center">
          <h2 className="text-[1rem] font-semibold text-[var(--p-texto)]">
            A leitura da fila falhou
          </h2>
