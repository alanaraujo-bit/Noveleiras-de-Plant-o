...
export default function ErroDaComunidade({
  error,
  retry,
}: {
  ...
  useEffect(() => {
    console.error("Falha ao carregar a comunidade no painel", error);
  }, [error]);

  return (
    <>
      <Cabecalho
        titulo="Comunidade indisponível"
        descricao="Não foi possível ler publicações, comentários e denúncias agora."
      />
      <Conteudo>
        <section className="painel-cartao px-5 py-10 text-center">
          <h2 className="text-[1rem] font-semibold text-[var(--p-texto)]">
            A leitura da comunidade falhou
          </h2>
