...
export default function ErroDaDescoberta({
  error,
  retry,
}: {
  ...
  useEffect(() => {
    console.error("Falha ao carregar o painel de descoberta", error);
  }, [error]);

  return (
    <>
      <Cabecalho
        titulo="Descoberta indisponível"
        descricao="Não foi possível consultar as buscas registradas agora."
      />
      <Conteudo>
        <section className="painel-cartao px-5 py-10 text-center">
          <h2 className="text-[1rem] font-semibold text-[var(--p-texto)]">
            A leitura das buscas falhou
          </h2>
