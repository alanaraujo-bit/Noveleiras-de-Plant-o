...
export default function ErroDoStreaming({
  error,
  retry,
}: {
  ...
  useEffect(() => {
    console.error("Falha ao carregar o painel de streaming", error);
  }, [error]);

  return (
    <>
      <Cabecalho
        titulo="Streaming indisponível"
        descricao="Não foi possível consultar os fatos de reprodução agora."
      />
      <Conteudo>
        <section className="painel-cartao px-5 py-10 text-center">
          <h2 className="text-[1rem] font-semibold text-[var(--p-texto)]">
            A leitura do streaming falhou
          </h2>
