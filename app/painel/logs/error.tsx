...
export default function ErroDosLogs({
  error,
  retry,
}: {
  ...
  useEffect(() => {
    console.error("Falha ao carregar a central de logs", error);
  }, [error]);

  return (
    <>
      <Cabecalho
        titulo="Logs indisponíveis"
        descricao="Não foi possível consultar a central de logs agora."
      />
      <Conteudo>
        <section className="painel-cartao px-5 py-10 text-center">
          <h2 className="text-[1rem] font-semibold text-[var(--p-texto)]">
            A leitura dos logs falhou
          </h2>
